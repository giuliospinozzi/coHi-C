...
##Versione in cui utilizzo dati provenienti da cartelle non aventi nel nome il suffisso "NORM"
...
do
    ...
    while IFS=$'\t' read -r sample ID path_dir T_UT T_Type Fastq1_Dir Fastq2_Dir
	  ...
	  cd ${arr[2]}
      mkdir -p Loops_Plots_${RESOLUTION}_all_chr/plotLoops_${RESOLUTION}_${coor}
	  ...
	  while IFS=":" read -r chr start_end; do
		while IFS="-" read -r start end; do
		      ...
		      # hicPlotTADs --tracks track.ini -o ../../TADs_Plots_tadcoor/plotTADs_${RESOLUTION}_${coor}/${arr[0]}_${RESOLUTION}_TADs_${chr}_${start}-${end}.png --region ${coor} --dpi 300
              hicPlotMatrix -m inter_30_${RESOLUTION}_corrected.h5  -o ../../Loops_Plots_${RESOLUTION}_all_chr/plotLoops_${RESOLUTION}_${coor}/${arr[0]}_${RESOLUTION}_loops_${chr}_${start}-${end}_matrix_loop.png --log1p --clearMaskedBins  --region ${coor} --loops loops.bedgraph --dpi 300
